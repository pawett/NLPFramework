...
import com.NLPFramework.Formatters.TreeTagFormatter;
 ...
 * @since 2011
 */
public class TreeTagger implements ITokenizer{
    ...
    public  TokenizedFile run(String originalFilename, TokenizedFile file) {
        try {
        	// sed 's/``/\"/g' | sed \"s/''/\\"+"\"/g\" | 
            String[] command = {"/bin/sh", "-c","cat \"" + originalFilename +"\" | sed \"s/\\([^[:blank:]]\\)-\\([^[:blank:]]\\)/\\1 - \\2/g\" | "+program_bin_tokenizer+" | sed \"s/[[:blank:]]\\+/\t/g\""};
            Process p = Runtime.getRuntime().exec(command);
            String outputfile = originalFilename + ".treetag";
            // TreeTagger Format
            // token  POS lemma (separated by \t)
                String line;
            BufferedWriter output = new BufferedWriter(new FileWriter(outputfile));
            ...
            try {
                ...
                while ((line = stdInput.readLine()) != null) 
                {
                	...
                	formatter.addFromPipes(line, new TreeTagFormatter());            	
                }
