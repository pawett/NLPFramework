...
import com.NLPFramework.Helpers.XmlAttribs;
...
public class Judgement implements Comparable<Judgement> {
    ...
    public Judgement(JudgementType j, String elem, String attrs, int nline, String keyline, String annotline){
        try{
    ...
    public void changeJudgement(JudgementType j){
        try{
        //judgements jdm=judgements.values()[j];  // Check judgement
        judgement=j;
        } catch (Exception e) {
    ...
    public void changeAndExtendJudgement(JudgementType j, String keyline, String annotline){
        try{
        if(keyline==null || annotline==null){
           throw new Exception("Null keyline or annotline");
        }
    ...
    public void add_alt_attribs(String attrs, String type){
        ...
        for(String key : attribs.keySet()){
            alt_annot_attribs_scores.put(key, JudgementType.miss.ordinal());
        }
        ...
        for(String key : alt_annot_attribs.keySet()){
            if(alt_annot_attribs_scores.get(key)!=null){
                if(attribs.get(key).equalsIgnoreCase(alt_annot_attribs.get(key))){
                    alt_annot_attribs_scores.put(key, JudgementType.corr.ordinal());
                }else{
                    alt_annot_attribs_scores.put(key, JudgementType.inco.ordinal());
                }
            }else{
                alt_annot_attribs_scores.put(key, JudgementType.spur.ordinal());
            }
        }
    ...
    public Boolean isAligned(){
       if(judgement==JudgementType.corr || judgement==JudgementType.inco){
           return true;
       }else{
           return false;
       }
