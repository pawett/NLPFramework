...
public class TestTimexClassification extends TestTMLBase implements IActionExecutor {


	private String elem = "timex";
	protected String filter;
	
	public TestTimexClassification(File train_dir,  File modelDir, String approach, String task,
			Language lang,
	...
	public void execute()
	{
		...
		try 
		{
			...
			{	
				...
				f.cleanAnnotations(TimeMLTrainTypes.classification);
				NLPProcessor nlP = new NLPProcessor(f);
				nlP.processTimex();
							
				//numEventsAnnotated += f.getNumTimex();
				Score score = new Score(elem, f.getName());
				scorer.scoreClassification(elem, null, score, f, kTmlFile);
				//"VB(?:D|N|P|G|Z)"
		...
		//	TokenizedFileHashtable featuresTestKeyFiles = new TokenizedFileHashtable(featuresTestPath);
			...
			Score score =  scorer.score(elem, TimeMLTrainTypes.classification.toString(), annotatedTestFiles, featuresTestKeyFiles, null);
			score.print("total");

			Logger.WriteDebug("VERB " + elem);
			score =  scorer.score(elem, TimeMLTrainTypes.classification.toString(), annotatedTestFiles, featuresTestKeyFiles, "VB");
			score.print("total");

			Logger.WriteDebug("ADJ " + elem);
			score =  scorer.score(elem, TimeMLTrainTypes.classification.toString(), annotatedTestFiles, featuresTestKeyFiles, "JJ");
			score.print("total");
			Logger.WriteDebug("NOUN " + elem);
			score =  scorer.score(elem, TimeMLTrainTypes.classification.toString(), annotatedTestFiles, featuresTestKeyFiles, "NN");
			score.print("total");
			Logger.WriteDebug("TOTAL " + elem);
			score =  scorer.score(elem, TimeMLTrainTypes.classification.toString(), annotatedTestFiles, featuresTestKeyFiles, null);
			score.print("total");
